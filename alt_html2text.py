...
if __name__ == "__main__":
  url = "http://thehinducrosswordcorner.blogspot.in/2013_11_01_archive.html"
	year = 2013
	month = 10
	y = '/2013_'
	f = open("t1.txt","a")
	while(year>2007):
		...
		year = year - 1
		y = '/' + str(year) + '_' 
		url = url.replace(yinit,y)
		m = '_11_'
		print "Currently in the year:" + str(year)
		while (month!=0):
			minit = m
			if (month<10):
				m ='_'+ '0' +  str(month) + '_'
				...
				m = '_'+ str(month)+ '_'		
			print "Currently going through month "+m
			url = url.replace(minit,m)
			print url
			month = month - 1
			req = urllib2.Request(url)
	#		print "here"+str(month)
	...
	#		print "data available"
		...
		#	end = html.find("Contributors")
			while(x!=0):
	#			print "Getting the clues"
				i = html.find("DOWN",i +1)
				x = x-1
				j = html.find("ACROSS",j+1)
				f.write(html[i:j])
		month = 10
		url = url.replace(m,'_11_')
